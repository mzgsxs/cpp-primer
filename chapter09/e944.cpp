...
(string & s, string const & oldVal, string const & newVal)
{
  ...
  while ( (cursor != s.end()) && ((cursor + (size_oldVal - 1)) != s.end()) ){
    string window(cursor, cursor+size_oldVal);
    cout << s << endl;
    cout << window << endl;
    if (window == oldVal){
      string::size_type pos = cursor - s.begin();
      s.replace(cursor, cursor + size_oldVal, newVal);
      cursor = s.begin() + (pos + size_newVal - 1);
      cout << *cursor<< endl;
    }
    ++cursor;
  }
...
int main(){
string s{"thotho thothoa string used for testing, tho and thru, tho, tho tho"},
       oldVal{"tho"},
       newVal{"though"};
       //oldVal{"thru"},
       //newVal{"through"};
